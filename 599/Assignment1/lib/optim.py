...
class SGDM(Optimizer):
    ...
    def step(self):
        ...
        #############################################################################
        #pass
        #self.velocity = self.net.params
        for layer in self.net.layers:
            for n, dv in layer.grads.items():
                #print(n)
                self.velocity[n] = self.momentum * self.velocity.get(n, 0.) - self.lr * dv
...
class RMSProp(Optimizer):
    ...
    def step(self):
        ...
        for layer in self.net.layers:
            for n, dv in layer.grads.items():
                self.cache[n] = self.decay * self.cache.get(n, 0.) + (1 - self.decay) * (dv ** 2)
...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for layer in self.net.layers:
            for n, dv in layer.grads.items():
                self.mt[n] = self.beta1 * self.mt.get(n, 0.) + (1. - self.beta1) * dv
                self.vt[n] = self.beta2 * self.vt.get(n, 0.) + (1. - self.beta2) * (dv ** 2)
